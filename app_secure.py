...
# Veritabanı dosya yolu (güvenli versiyon için ayrı dosya)
...
def log_query(user_input, executed_query):
    ...
    Güvenli versiyonda parametreli sorgu kullanıldığını belirtir.
...
def index():
    ...
    # Giriş yapmamışsa güvenli login sayfasını göster
...
def login():
    ...
    # Kullanıcı girdisi parametre olarak geçiriliyor - SQL Injection açığı yok!
    ...
    try:
        ...
        # Kullanıcı girdisi SQL kodu olarak değil, veri olarak işlenir
...
def menu():
    ...
    try:
        ...
        # Güvenli menü sayfasını render et
...
def search():
    ...
    try:
        ...
        # Güvenli arama sonuçları sayfasını render et
...
def show_database():
    ...
    try:
        ...
        # Güvenli veritabanı sayfasını render et
        return render_template('database_secure.html', db_data=db_data, tables=tables, user=session, is_admin=is_admin)
    except sqlite3.Error as e:
        # Veritabanı hatası durumunda hata mesajı göster
        flash(f'Veritabanı hatası: {str(e)}', 'error')
        return redirect(url_for('menu'))
